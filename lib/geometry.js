...
const METROPOLITAN_MEASURE_BBOX_OVERRIDES = new Set(["FRA", "NLD"]);
...
function countGeometryVertices(feature) {
  let count = 0;
...
export function getCountryVisibleScreenAnchor(country, projectLngLat, viewportRect) {
  ...
  const vertexCount = countGeometryVertices(country.feature);
  ...
  walkCoords(country.feature.geometry.coordinates, (lng, lat) => {
  ...
  const averaged = averagePoints(visiblePoints);
  if (averaged) return averaged;

  const screenBounds = getCountryScreenBounds(country, projectLngLat);
  if (screenBounds) {
    const visibleBounds = intersectScreenRects(screenBounds, viewportRect);
    if (visibleBounds) {
      return {
