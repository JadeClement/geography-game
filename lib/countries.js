...
import { displayName, GEOJSON_PATH, resolveIso3 } from "./constants";
import {
  ...
  const countries = geojson.features
    .map((feature) => {
      ...
      return {
        ...
        rawName,
        isSmall: isSmallCountry(enrichedFeature),
...
export function isCorrectCountry(clicked, target) {
  ...
  return (
    ...
    displayName(clicked.name).trim().toLowerCase() ===
      displayName(target.name).trim().toLowerCase()
  );
}
