 ...
 *   population/area/landlocked/languages/neighbors[iso3]/region). `id` is
...
import { QUESTION_TIERS, QUESTION_TYPES } from "@/lib/learn/questionTypes";
import {
  getAreaPeers,
  getPopulationPeers,
  isBlockedPair,
...
// Mid-size "select all that apply" neighbor list: several real borders + distractors.
const SELECT_ALL_LIST_SIZE = 7;
const SELECT_ALL_MIN_CORRECT = 2;
const SELECT_ALL_MAX_CORRECT = 4;
...
export function generateNeighborSelectAll(country, allCountries) {
  ...
  // Cap how many neighbors appear in the list so mid-size countries (and Russia /
  // China) don't explode the option count; leave room for ≥2 distractors.
  const maxCorrect = Math.min(
    SELECT_ALL_MAX_CORRECT,
    neighbors.length,
    SELECT_ALL_LIST_SIZE - 2
  );
  const correctRecords = shuffle(neighbors).slice(0, maxCorrect);
  const correctIds = correctRecords.map((record) => cid(record));
  const distractorsNeeded = SELECT_ALL_LIST_SIZE - correctRecords.length;
  ...
  const options = shuffle([...correctRecords, ...distractors].map(countryOption));

  return baseQuestion(QUESTION_TYPES.NEIGHBOR_SELECT_ALL, country, {
    prompt: `Which of these countries border ${country.name}?`,
    promptSubtext: "Select all that apply.",
    answerType: "multi_select",
    // Exact set the learner must select (subset of neighbors shown in options).
...
export function generateLanguageFamily(country, allCountries) {
  ...
  const correct = shuffle(languages)[0];
  const options = shuffle([
  ...
  return baseQuestion(QUESTION_TYPES.LANGUAGE_FAMILY, country, {
    prompt: `Which language is commonly spoken in ${country.name}?`,
    answerType: "multiple_choice",
...
export function generateQuestion(typeId, country, allCountries, masteryStats) {
  ...
  return generator(country, allCountries, masteryStats) ?? null;
}
