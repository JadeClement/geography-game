...
/** Prefer a highlight, then geography, then any other standard fact. */
export function getDidYouKnowFact(facts) {
  const { standardFacts, highlights } = partitionCountryFacts(facts);
  if (highlights[0]?.text) return highlights[0].text;

  const geography = standardFacts.find((fact) => fact.category === "geography");
  if (geography?.text) return geography.text;

  return standardFacts[0]?.text ?? null;
}
