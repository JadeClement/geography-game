...
function fallbackLanguages(languageMap) {
  if (!languageMap || typeof languageMap !== "object") return [];
  return Object.values(languageMap).filter(Boolean).slice(0, 2);
}
...
async function main() {
  ...
  const [dr5hnResponse, mledozeResponse, cldrSources] = await Promise.all([
    fetch(dr5hnUrl),
    fetch(mledozeUrl),
    loadCldrLanguageSources(),
  ]);
  ...
  const { territoryInfo, languageNames } = cldrSources;
    ...
    .map((feature) => {
      ...
      const population =
        typeof dr5hn?.population === "number" && dr5hn.population > 0
          ? dr5hn.population
          : null;
      const alpha2 = resolveAlpha2(iso3, mledoze?.cca2);
