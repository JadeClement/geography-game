...
async function fetchCountriesGeoJSON() {
  ...
  const countries = geojson.features
    .map((feature) => {
      ...
      if (!manifest || !manifest.enabled) {
        ...
        // Any other non-playable land becomes neutral hatched base land.
        baseLandFeatures.push(landFeature);
        return null;
      }
  ...
  displayMapCountries.sort((a, b) => a.name.localeCompare(b.name));

  return {
    countries,
    /** Non-playable land that should still appear on region / mastery maps. */
    displayMapCountries,
    geojson: {
