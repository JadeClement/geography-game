...
/** Cover-scale so the image fills a square viewport of the given size. */
export function getAvatarCoverScale(imageWidth, imageHeight, viewportSize) {
  return viewportSize / Math.min(imageWidth, imageHeight);
}

/**
 * Clamp pan offsets so the circular viewport always stays inside the image.
 * Offsets are in viewport pixels relative to the image center.
 */
export function clampAvatarCropOffset(offsetX, offsetY, imageWidth, imageHeight, viewportSize, zoom) {
  const displayScale = getAvatarCoverScale(imageWidth, imageHeight, viewportSize) * zoom;
...
/**
 * Load a user-selected image as a data URL, downscaling very large photos so
 * the cropper stays responsive while keeping enough resolution for a 256px avatar.
 */
export async function loadImageFileForCrop(file, { maxDimension = 1600, onProgress } = {}) {
...
/**
 * Export the circular crop area as a square JPEG avatar.
 * `offsetX` / `offsetY` / `zoom` match the cropper's viewport transform.
 */
export async function cropAvatarImage(
  imageSrc,
  { offsetX = 0, offsetY = 0, zoom = 1, viewportSize = 280, outputSize = 256, quality = 0.85 } = {}
) {
  const image = await loadHtmlImage(imageSrc);
  const { offsetX: x, offsetY: y } = clampAvatarCropOffset(
    offsetX,
    offsetY,
    image.naturalWidth,
    image.naturalHeight,
  ...
  const displayScale = getAvatarCoverScale(image.naturalWidth, image.naturalHeight, viewportSize) * zoom;
  const sourceSize = viewportSize / displayScale;
  const sourceX = image.naturalWidth / 2 - x / displayScale - sourceSize / 2;
  const sourceY = image.naturalHeight / 2 - y / displayScale - sourceSize / 2;
...
export async function resizeImageFile(file, { maxSize = 256, quality = 0.85, onProgress } = {}) {
  if (!file?.type?.startsWith("image/")) {
    throw new Error("Choose a JPEG, PNG, or WebP image.");
  }
