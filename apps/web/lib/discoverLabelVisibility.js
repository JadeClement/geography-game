...
export function isDiscoverLabelVisible({
  ...
  isAnimating,
  /**
   * Skip fit/occlusion hiding (flags shrink instead; learn border reveals opt
   * in so every neighbor title stays visible). Still requires an on-screen
   * country anchor — off-map countries never show a label.
   */
  alwaysShow = false,
}) {
