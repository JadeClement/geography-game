...
export function getGoalCopy(ctx) {
  ...
  const speedNote = ctx.isLearning ? "" : " as fast as you can";

  return {
    title: "Your goal",
    body:
      ctx.mode === GAME_MODES.FLAGS
        ? `Find the country whose flag matches the prompt, then click it on the map${speedNote}.`
        : ctx.mode === GAME_MODES.CAPITALS
          ? `Find the country whose capital matches the prompt, then click it on the map${speedNote}.`
          : `Find the country whose name matches the prompt, then click it on the map${speedNote}.`,
    titleMobile: "Your goal",
    bodyMobile:
      ctx.mode === GAME_MODES.FLAGS
        ? `Match the flag shown in the floating prompt to a country on the map, then tap it${speedNote}.`
        : `Match the ${item} in the prompt to a country on the map, then tap it${speedNote}.`,
  };
}
